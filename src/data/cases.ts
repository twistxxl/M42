...
export const cases: CaseItem[] = [
  {
    id: 'abuse',
    cat: 'Веб-приложения',
    title: 'Интернет-магазин с нуля вместо продаж через сообщения',
    ...
    fullTask:
      'Бренд Anti-Abuse вёл продажи через группу ВКонтакте и Instagram: каждая продажа — это переписка и ручное оформление. Нужен был полноценный магазин который работает на бренд и продаёт без участия менеджера.',
    done: [
    ...
    stack: 'Интернет-магазин, приём оплат, аналитика, Яндекс.Метрика',
    metrics: [
    ...
    en: {
      title: 'Online store built from scratch instead of selling through messages',
      task: 'Sales ran through VKontakte and Instagram — there was no store of their own',
      result: 'Conversion grew from 2.1% to 10.4%',
      fullTask:
        "The Anti-Abuse brand sold through a VKontakte group and Instagram: every sale meant a conversation and manual processing. It needed a full-fledged store that works for the brand and sells without a manager involved.",
      done: [
      ...
      stack: 'Online store, payments, analytics, Yandex Metrica',
      metrics: [
  ...
  },
  {
    ...
    task: 'Администраторы не справлялись с звонками и их обработкой',
    result: '80% записей ушло в бота',
    ph: 'скриншот: сайт Barbarossa',
    img: '/images/barbarossa.png',
    url: 'https://barbarossa.top/',
    fullTask:
      'В сети из четырёх барбершопов администраторы половину смены отвечали на звонки о записи и переносах. Часть клиентов не дозванивались и уходили к конкурентам.',
    done: [
    ...
    en: {
      ...
      fullTask:
        "A chain of four barbershops: admins spent half their shift answering calls about bookings and reschedules, some clients couldn't get through and left for competitors.",
      done: [
  ...
  },
  {
    ...
    task: 'MVP не справлялся: новая сезонная коллекция — правки в коде. Сайт падал под нагрузкой',
    result: 'Простое управление через админ-панель. Оптимизация слабых мест, чтобы не было падений сервера',
    ph: 'скриншот: интерфейс магазина Ravir',
    img: '/images/ravir.png',
    url: 'https://ravir.store/',
    fullTask:
      'У основателя уже был работающий MVP — но каждая новая сезонная коллекция требовала правок в коде. В день запуска сайт падал под нагрузкой. Управлять каталогом без разработчика было невозможно. Нужен был удобный инструмент — и для клиента, и для владельца.',
    done: [
      { text: 'Переделал клиентский интерфейс под удобство покупки' },
      { text: 'Выстроил полноценную админ-панель для управления каталогом без разработчика' },
      { text: 'Оптимизировал архитектуру под нагрузку в дни ажиотажа' },
      { text: 'Сезонные запуски теперь делаются через панель за минуты' },
    ],
    stack: 'Веб-приложение, панель администратора, каталог товаров',
    metrics: [
      { n: '10 мин', l: 'запуск новой сезонной коллекции через админ-панель' },
      { n: '0', l: 'падений сайта при сильной нагрузке' },
      { n: '−70%', l: 'времени на управление каталогом' },
    ],
    en: {
      title: 'Relaunching an online store: convenient for both customer and owner',
      task: "The MVP couldn't keep up: every new seasonal collection meant code changes, and the site crashed under load",
      result: "Simple management through the admin panel. Weak points optimized so the server doesn't go down",
      fullTask:
        "The founder already had a working MVP — but every new seasonal collection required code changes. On launch day the site crashed under load. Managing the catalog without a developer was impossible. It needed a tool that's convenient for both the customer and the owner.",
      done: [
        ...
        { text: 'Optimized the architecture for high-demand launch days' },
        { text: 'Seasonal launches are now done through the panel in minutes' },
      ],
      stack: 'Web app, admin panel, product catalog',
      metrics: [
        { n: '10 min', l: 'launching a new seasonal collection via the admin panel' },
        { n: '0', l: 'site crashes under heavy load' },
  ...
  },
  {
    id: 'jtstudy',
    cat: 'Сайты',
    title: 'Реклама у блогеров работала. Страница — нет',
    task: 'Люди кликали, читали и закрывали. Деньги на рекламу уходили впустую',
    result: 'Рекламный бюджет вернулся за 2 недели — и начал приносить прибыль',
    ph: 'скриншот: лендинг JTStudy',
    img: '/images/jtstudy.png',
    fullTask:
      'Преподаватель английского из Казахстана платила блогерам и получала клики. Люди заходили на страницу, листали до конца — и уходили. Деньги тратились, новых учеников не было. Проблема оказалась не в рекламе: страница рассказывала что будет на курсе, но не отвечала на главный вопрос покупателя — что конкретно изменится в моей жизни?',
    done: [
      { text: 'Переписал страницу: вместо программы курса — результат который получит ученик' },
      { text: 'Сделал так чтобы страница нормально открывалась с телефона — большинство переходило из Instagram' },
      { text: 'Убрал регистрацию перед оплатой — теперь оплатить можно в один шаг' },
    ],
    stack: 'Лендинг, приём оплат, аналитика',
    metrics: [
      { n: '2 нед', l: 'и рекламный бюджет уже вернулся' },
      { n: 'каждый 20й', l: 'посетитель страницы платил за курс' },
      { n: 'вдвое', l: 'дешевле стала обходиться каждая заявка' },
    ],
    en: {
      title: "Blogger ads worked. The landing page didn't",
      task: 'People clicked, read, and left. Ad money was going to waste',
      result: 'Ad budget paid back in 2 weeks — and started turning a profit',
      fullTask:
        "An English teacher from Kazakhstan was paying bloggers and getting clicks. People landed on the page, scrolled to the end — and left. Money was spent, no new students came in. The problem wasn't the ads: the page described what the course covered, but never answered the buyer's real question — what will actually change in my life?",
      done: [
        { text: "Rewrote the page: swapped the course curriculum for the result the student gets" },
        { text: 'Made the page work properly on mobile — most traffic came from Instagram' },
        { text: 'Removed registration before payment — checkout is now a single step' },
      ],
      stack: 'Landing page, payments, analytics',
      metrics: [
        { n: '2 wks', l: 'and the ad budget had already paid back' },
        { n: '1 in 20', l: 'page visitors paid for the course' },
        { n: '2×', l: 'cheaper cost per lead' },
      ],
    },
  },
  {
    ...
    task: 'Заказы принимали только по телефону, некоторые клиенты не дозванивались',
    result: 'Вдвое меньше пропущенных заказов, первые онлайн-заявки в день запуска',
    ...
    en: {
      ...
      task: "Orders were phone-only, and some customers couldn't get through",
      result: '-50% missed orders, first online orders on launch day',
      fullTask:
  ...
  },
  {
    ...
    result: 'Воронка продаж на английском в виде лендинга запущена за 3 недели',
    ...
    en: {
      ...
      result: 'English-language sales funnel launched as a landing page in 3 weeks',
      fullTask:
  ...
  },
  {
    ...
    task: 'У основателя была идея — не было понимания как превратить её в продукт',
    result: 'Сделанное MVP за 3 месяца',
    ...
    metrics: [
      ...
      { n: '3', l: 'платформы после: сайт, расширение в браузере, мобильное приложение' },
    ...
    en: {
      ...
      task: 'The founder had an idea — no understanding of how to turn it into a product',
      result: 'MVP built in 3 months',
      fullTask:
      ...
      metrics: [
        ...
        { n: '3', l: 'platforms afterward: website, browser extension, mobile app' },
  ...
  },
  {
    ...
    task: 'Основателю нужен был маркетплейс с нуля',
    result: 'Полноценный маркетплейс за 12 месяцев. Проект окупился в первый год',
    ...
    en: {
      ...
      task: 'The founder needed a marketplace built from scratch',
      result: 'Full marketplace in 12 months. The project paid back in the first year',
      fullTask:
  ...
  },
  {
    ...
    task: 'Некуда было вести клиента — менеджеры уставали от ручной обработки звонков',
    ...
    en: {
      ...
      task: 'Nowhere to send clients — managers were exhausted from manually processing calls',
  ...
  },
  {
    ...
    task: 'Автор контента не знал, как монетизировать свои навыки в сфере моды и брендопостроения',
    result: 'Бот + сайт за 2 месяца. Автор начал монетизировать свою аудиторию',
  ...
  },
  {
    ...
    task: 'Бизнесы вокруг вузов тратили деньги на рекламу которую студенты не замечали',
    ...
    fullTask:
      'Вокруг любого университета — кафе, барбершопы, репетиторы, курсы. Всем нужны клиенты. Но студент не читает листовки у входа и не замечает баннеры. Он живёт в телефоне. Нужна была платформа которую студент откроет сам — а бизнес получит к нему прямой доступ через рекламу.',
    done: [
